...
class DataLoader(object):
    ...
    def __init__(self, image_paths: List[str], mask_paths: List[str], image_size: Tuple[int],
                 ...
                 augment: bool = True, compose: bool = False, one_hot_encoding: bool = True, palette=None):
        """
    ...
    @tf.function
    def _augmentation_func(self, image_f, mask_f):
        if self.augment:
            if self.compose:
    ...
    def data_batch(self, batch_size, shuffle=False):
        ...
        data = data.map(self._map_function, num_parallel_calls=AUTOTUNE).map(self._augmentation_func)
